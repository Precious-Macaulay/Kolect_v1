  ...
  return (
    <div className="h-screen w-screen p-special-m">
      ...
      <div className="bg-white w-full m-special-xl rounded-special p-special-m">
        <h6>Owed Funds</h6>
        <p className="amount">{`NGN ${
          userData?.owed ? (userData?.owed / 100).toFixed(2) : `0.00`
        }`}</p>
      </div>
      <div className="bg-white w-full m-special-x rounded-special p-special-m">
        <h6>Reserved Funds</h6>
        <p className="amount">{`NGN ${
          userData?.reserved ? (userData?.reserved / 100).toFixed(2) : `0.00`
        }`}</p>
      </div>
      <div className="m-special-xl">
        ...
        <div className="flex w-full justify-between item-center m-special-xl gap-special">
          <Button
            ...
            as={Link}
            className="h-60 flex flex-col justify-center items-center quick-link-button"
          >
            <AiOutlineSend />
      ...
      <div className="m-special-xl">
        ...
        {/* 
                {userData?.transactions?.map((item) => (
                    <div key={item.amount} className="bg-white w-full m-special-xl p-special-tiny flex justify-between items-center">
                        <div><h1 className="amount">{`NGN ${(item.amount/100).toFixed(2)}`}</h1><p className="light-text">{item.customer}</p></div>
                        <div><p className="light-text">{item.date}</p></div>
                    </div>
                ))} */}
        {/* no transaction data */}
